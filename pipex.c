...
/*   Updated: 2025/08/04 18:02:31 by oamairi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	first(char *file_in, char *file_out, t_pipex_content content)
{
	pid_t	fils;
	...
	if (fils == 0)
	{
		...
		second(file_out, content);
	(waitpid(fils, NULL, 0), close(content.file_in), close(content.pip[1]));
	return (1);
}
...
int	main(int argc, char **argv, char **env)
{
	...
	if (first(argv[1], argv[4], content) == 2)
